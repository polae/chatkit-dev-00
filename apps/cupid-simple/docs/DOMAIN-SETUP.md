# Cupid-Simple Domain Configuration - Production Setup
...
2. Add A record:
   - **Type:** A
   - **Name:** `cupid-simple` (for cupid-simple.yourdomain.com)
   - **Value:** `3.136.156.43` (your EC2 IP)
   - **TTL:** 600 (or default)
3. Save changes
...
2. Click **Add domain**
3. Enter your domain: `cupid-simple.yourdomain.com`
4. Click **Save**
...
```bash
ssh -i ~/.ssh/cupid-simple-pk-00.pem ubuntu@3.136.156.43
...
FRONTEND_URLS=https://cupid-simple.yourdomain.com,http://3.136.156.43
...
The code has been updated to support the production domain:

```bash
cd ~/chatkit-dev-00
git pull origin development
cd apps/cupid-simple
```

**Changes include:**
- Updated Caddyfile for automatic HTTPS
- Added domain to vite.config.ts allowedHosts
- Added FRONTEND_URLS environment variable support
...
**Via Domain (HTTPS):**
```bash
curl -I https://cupid-simple.yourdomain.com
```

**In Browser:**
- Open: `https://cupid-simple.yourdomain.com`
- The chat interface should now load
- SSL certificate will be automatically provisioned by Caddy

**Fallback via IP (HTTP):**
- `http://3.136.156.43` should still work
...
```bash
nslookup cupid-simple.yourdomain.com
```
...
Internet
   ↓
DNS (cupid-simple.yourdomain.com → 3.136.156.43)
   ↓
Caddy (Port 443/80)
...
**Access your app at:** `https://cupid-simple.yourdomain.com`
